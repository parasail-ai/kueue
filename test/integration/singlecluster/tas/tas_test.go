...
var _ = ginkgo.Describe("Topology Aware Scheduling", ginkgo.Ordered, func() {
	...
	ginkgo.When("Single TAS Resource Flavor", func() {
		...
		ginkgo.When("Nodes are created before test with the hostname being the lowest level", func() {
			...
			ginkgo.It("should keep the TopologyAssignment of a single-Pod-owned workload when node fails and TASSkipReplacementForPlacedPod is enabled", framework.SlowSpec, func() {
				features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.TASSkipReplacementForPlacedPod, true)
				var wl1 *kueue.Workload
				nodeName := nodes[0].Name
			...
			ginkgo.It("should update the TopologyAssignment of a Job-owned workload when node fails and TASSkipReplacementForPlacedPod is enabled", framework.SlowSpec, func() {
				features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.TASSkipReplacementForPlacedPod, true)
				var wl1 *kueue.Workload
				nodeName := nodes[0].Name
